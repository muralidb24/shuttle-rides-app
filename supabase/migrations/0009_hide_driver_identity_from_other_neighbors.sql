-- Privacy: when a ride gets covered by one neighbor, the other neighbors
-- whose pending offers get auto-declined should NOT be told who took it -
-- just that it's covered. (The requester's own "Ride confirmed"
-- notification still names the driver, since the requester needs that to
-- coordinate the actual pickup.)
...
begin
  ...
  loop
    ...
    values (
      ...
      'Another neighbor has already committed to give ' || v_requester_name || ' a ride for the ' || v_shuttle_time || ' shuttle on ' || v_shuttle_date || '. You don''t need to do anything.',
      v_request_id
    );
  end loop;
end;
